...
#            torch.cuda.empty_cache()
            if args['reg']:
                ...
                out.append([float(roc_score[0].cpu().numpy()), int(roc_score[0] >= args['prop'])])
            # write_check = 0
    ...
    if args['reg']:
        ...
        dff_top_k.to_csv(top_k_file, index=False)
        # f.close()
def screen(file='', sep=',', models=None,prop=0.5, smiles_col='Smiles',out_file=None,tasks=1,top_k=10, task_type="cla"):
