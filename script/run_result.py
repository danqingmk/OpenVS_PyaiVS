...
def model_set(X,Y,split_type='random',FP_type='ECFP4',model_type = 'SVM',model_dir=False,file_name=None,task_type='cla',difftasks=None):
    if model_type =='SVM':
        para_svm(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,task_type=task_type)
    elif model_type =='RF':
        para_rf(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,task_type=task_type)
    elif model_type =='KNN':
        para_knn(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,task_type=task_type)
    elif model_type == 'XGB':
        para_xgb(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,task_type=task_type)
    elif model_type == 'DNN':
        para_dnn(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,file_name=file_name,difftasks=difftasks,task_type=task_type)
    elif model_type == 'gat':
...
def main(file,split,FP,models,cpus,mpl,device,task_type="cla"):
    # rfile=file.replace(file.split('/')[-1], 'record_every_model.csv')
