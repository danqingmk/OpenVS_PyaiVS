...
def cir_file(file=None, sep=',', models=None, prop=0.5, out_file="./result_screen_all.csv", smiles_col='smiles', top_k=10, task_type="cla"):
    data=pd.read_csv(file)
    sm=list(data[smiles_col])
    top_k_file=out_file.replace('.csv', '_top_{}.csv'.format(top_k))
    tp = models.split('/')[-1].split('_')[2]
    model = joblib.load(models)
    out=[]
    if task_type == "cla":
        for s in tqdm(sm):
            try:
                fprint = fp([s], type=tp, file=file)
                pred = model.predict_proba(fprint)
                out.append([pred[0][1], int(pred[0][1] >= prop)])
            except:
                out.append([0, 0])
