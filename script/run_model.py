...
def model_set(X,Y,split_type='random',FP_type='ECFP4',model_type = 'SVM',model_dir=False,file_name=None,task_type='cla',difftasks=None):
    # print("#####model_set：", task_type)
    if model_type =='SVM':
        tvt_svm(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,task_type=task_type)
    elif model_type =='RF':
        tvt_rf(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,task_type=task_type)
    elif model_type =='KNN':
        tvt_knn(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,task_type=task_type)
    elif model_type == 'XGB':
        # print("################## run XGB ##################")
        tvt_xgb(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,task_type=task_type)
    elif model_type == 'DNN':
        tvt_dnn(X, Y, split_type=split_type, FP_type=FP_type,model_dir=model_dir,file_name=file_name,difftasks=difftasks,task_type=task_type)
    elif model_type == 'gat':
...
def pair_param(file_name,data,split_type,model_type,FP_type,difftasks,task_type="cla"):
    # print("#####pair_param：", task_type)
    if model_type in ['SVM','KNN','RF','XGB']:
        if len(difftasks) == 1:
            ...
            if os.path.exists(paramname):
                ...
                model_set(X,Y, split_type=split_type,model_type=model_type,model_dir=model_dir,file_name=file_name,FP_type=FP_type,task_type=task_type)#
        ...
        if not os.path.exists(param_dir):
            os.makedirs(param_dir)
        paramname = param_dir + '/{}_{}_{}_{}.param'.format(split_type,task_type,FP_type,model_type)
        if os.path.exists(paramname):
            print(paramname, 'has done')
            pass
        else:
            ...
            model_set(X, Y, split_type=split_type, model_type=model_type, model_dir=model_dir,
                                   file_name=file_name, FP_type=FP_type, difftasks=difftasks,task_type=task_type)  #
        ...
        if not os.path.exists(param_dir):
            os.makedirs(param_dir)
        paramname = param_dir + '/{}_{}_{}.param'.format(split_type, task_type, model_type)
        if os.path.exists(paramname):
            print(paramname, 'has done')
            pass
        else:
            model_set(X, Y, split_type=split_type, model_type=model_type, model_dir=model_dir,
                                   file_name=file_name, difftasks=difftasks,task_type=task_type)
    return
...
def main(file,split,FP,models,cpus,mpl,device,task_type="cla"):
    # print("#####main：", task_type)
    start = time.time()
    ...
    if mpl == False:
        for i in range(len(ml.index.values)):
            ...
            print(f"########## Running {model_type}_{FP_type}_{split_type}")
            a = pair_param(file, data, split_type, model_type, FP_type,difftasks,task_type=task_type)
        ...
        for i in range(len(ml.index.values)):
            ...
            model_type = ml.iloc[i].model
            print(f"########## Running {model_type}_{FP_type}_{split_type}")
            result = p.apply_async(pair_param,args=(file,data,split_type,model_type,FP_type,difftasks,task_type))
    ...
    for i in range(len(dl.index.values)):
        ...
        start_time = time.time()
        a = pair_param(file, data, split_type, model_type, device, difftasks,task_type=task_type)
        use_time = int(time.time() - start_time)
        if use_time >= 1:
            local_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
            record_save = file.replace(file.split('/')[-1], 'record_every_model.csv')
