...
def model_screen(beat_settings=None, screen_file=None, prop=0.5, sep=',', smiles_col=None, output_file=None, top_k=10, task_type="cla"):
    if beat_settings is None:
        model,FP,split,model_dir,key=None,None,None,None,None
    else:
        model,FP,split,model_dir,key=beat_settings["model"],beat_settings["FP"],beat_settings["split"],beat_settings["model_dir"],beat_settings["key"]
    ...
    if output_file is not None:
        ...
        output_file = os.path.join(out_dir, screen_file.split('/')[-1].replace('.csv', '_screen_all.csv'))
    if model in ['SVM', 'KNN', 'RF', 'XGB']:
        ...
        # else:
            # seed = seq[seq['type'] == 'te'].sort_values('rmse', ascending=False)['seed'].tolist()[0]
        seed = seq[seq['type'] == 'te'].sort_values(key, ascending=False)['seed'].tolist()[0]
        
        if seed == 0:
            model_file = '_'.join([split, task_type, FP, model, 'bestModel.pkl'])            
            ...
            model_file = '_'.join([split, task_type, FP, str(seed), model, 'bestModel.pkl'])            
            model_path = os.path.join(os.path.join(model_dir, model), model_file)
            assert os.path.exists(model_path), 'no such model_path {}'.format(model_path)
        ...
        if os.path.isdir(screen_file):
            ...
            for file_content in os.listdir(screen_file):
                ...
                param = {'file': file_path, 'sep': sep, 'models': model_path, 
                        'prop': prop, 'out_file': output_file, "top_k": top_k, "task_type":task_type}
                get = p.apply_async(ml_screener.cir_file, kwds=param)
            p.close()
            p.join()
        elif os.path.isfile(screen_file):
            print(screen_file, model_path, smiles_col)
            ml_screener.cir_file(file=screen_file, sep=sep, models=model_path, prop=prop, out_file=output_file,
                                 smiles_col=smiles_col, top_k=top_k, task_type=task_type)
        else:
            print('What\'s this ?')
    elif model == 'DNN':
        param_dir = model_dir.replace('model_save', 'param_save')
        param_file = os.path.join(os.path.join(param_dir, model), '_'.join([split, task_type, FP, model + '.param']))
        ...
        if seed == 0:
            model_file = '_'.join([task_type, FP, split, "dataset", '%.4f' % parameter['dropout'],
        ...
        if os.path.isdir(screen_file):
            for file_content in os.listdir(screen_file):
                ...
                file_path = os.path.join(screen_file, file_content)
                param = {'file': file_path, 'sep': sep, 'models': model_path, 'prop': prop, 'out_file': output_file,
                         'smiles_col': smiles_col, 'top_k':top_k, 'task_type': task_type}
                get = p.apply_async(dnn_screener.screen, kwds=param)
                p.close()
                p.join()
        elif os.path.isfile(screen_file):
            dnn_screener.screen(file=screen_file, sep=sep, models=model_path, prop=prop, out_file=output_file,
                                smiles_col=smiles_col,top_k=top_k, task_type=task_type)
        else:
            print('What\'s this ?')
    else:
        param_dir = model_dir.replace('model_save', 'param_save')
        param_file = os.path.join(os.path.join(param_dir, model), '_'.join([split, task_type, model + '.param']))
        parameter = eval(open(param_file, 'r').readline().strip())
        result_dir = model_dir.replace('model_save', 'result_save')
        ...
        if seed == 0:
            if model == 'gcn':
                model_file = '_'.join([model, split, task_type, str(parameter['l2']), '%.6f' % parameter['lr'],
        ...
        if os.path.isdir(screen_file):
            for file_content in os.listdir(screen_file):
                ...
                file_path = os.path.join(screen_file, file_content)
                param = {'file': file_path, 'sep': sep, 'models': model_path, 'prop': prop, 'out_file': output_file,
                         'smiles_col': smiles_col, 'opt_res':parameter, 'top_k':top_k, 'task_type': task_type}
                get = p.apply_async(dl_screener.screen, kwds=param)
                p.close()
                p.join()
        elif os.path.isfile(screen_file):
            print(out_dir, screen_file)
            dl_screener.screen(file=screen_file, sep=sep, models=model_path, prop=prop, out_file=output_file,
                               smiles_col=smiles_col, opt_res=parameter, top_k=top_k, task_type=task_type)
        else:
            print('What\'s this ?')
